...
EmitTailCallVM(IonCode *target, MacroAssembler &masm, uint32_t argSize)
{
    ...
    // Compute frame size.
    masm.movl(BaselineFrameReg, s1);
    masm.addl(Imm32(BaselineFrame::FramePointerOffset), s1);
    masm.subl(BaselineStackReg, s1);

    // Store frame size without VMFunction arguments for GC marking.
    masm.movl(s1, s0);
    masm.subl(Imm32(argSize), s0);
    masm.store32(s0, Operand(BaselineFrameReg, BaselineFrame::reverseOffsetOfFrameSize()));

    // Push frame descriptor and perform the tail call.
    masm.makeFrameDescriptor(s1, IonFrame_BaselineJS);
    masm.push(s1);
...
{
    EmitCreateStubFrameDescriptor(masm, s1);
    masm.push(s1);
    masm.call(target);
}
